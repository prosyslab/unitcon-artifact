...
def main():
    ...
    parser.add_argument("--results", type=pathlib.Path, default=os.path.join(os.getenv("UNITCON_HOME"), "results"))
    args = parser.parse_args()

    if args.tool == "unitcon-rc1":
        create_unitcon_csv(args.results)
    elif args.tool == "unitcon-rc3":
        create_unitcon_strategies(args.results)
    else:
        create_csv(args.tool, args.iteration, args.results)
