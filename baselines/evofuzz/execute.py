...
BIN = os.path.join(os.path.dirname(__file__), "evofuzz-1.2.1.jar")
JAVA_MEM = 8000


def reset_evofuzz_results(dirpaths, force):
...
def validate(config, basedir, output_dir, classpaths):
    ...
    if not classpaths:
        command = [
            "source", "evofuzz-files/evofuzz.properties;", "javac", "-cp", f"$CP:{BIN}",
            f"{testdir}/*.java"
        ]
    else:
    ...
    if not classpaths:
        command = [
            "source", "evofuzz-files/evofuzz.properties;", "java", "-cp", f"$CP:{BIN}:{output_dir}"
        ]
    else:
...
def execute_evofuzz(config, basedir, output_dir, flags):
    # reset previous results if exists
    project_dir = os.path.join(basedir, config.project_dir)
    if not reset_evofuzz_results([output_dir], flags.force):
    ...
    logger.info(f"execute evofuzz project: {config.project}")
